    ...
    class UserEmployee : User
    {
        ...
        private void CheckShots(Animal animal)
        {
            ...
            if(shotInfo.Count > 0)
            {
                ...
                {
                    Query.UpdateShot("booster", animal);//write
                }
            }
            else
            {
        ...
        private void UpdateAnimal(Animal animal, Dictionary<int, string> updates = null)
        {
            ...
            {
                Query.EnterAnimalUpdate(animal, updates);//write
            }
            else
            {
        ...
        private IQueryable<Animal> SearchForAnimal(int iD)//write
        {
        ...
        {            
            var animals = Query.SearchForAnimalByMultipleTraits().ToList();//write
            if (animals.Count > 1)
            {
            ...
            {
                Query.RemoveAnimal(animal);//write
            }
        }
        private void AddAnimal()
        {
            ...
            animal.CategoryId = Query.GetCategoryId();//write
            ...
            animal.DietPlanId = Query.GetDietPlanId();//write
            Query.AddAnimal(animal);//write
        }
        protected override void LogInPreExistingUser()
        {
            ...
            {
                Console.Clear();
                employee = Query.EmployeeLogin(userName, password);//write
        ...
        private void CreateNewEmployee()
        {
            ...
            try
            {
                employee = Query.RetrieveEmployeeUser(email, employeeNumber); //write query
            }
            catch
            {
        ...
        private void UpdateEmployeeInfo()
        {
            GetUserName();
            GetPassword();
            Query.AddUsernameAndPassword(employee);//write
        }
        ...
        private void GetUserName() //write 
        {
            Console.Clear();
